...
function setColorFunc (keys) {
  ld.forEach(keys, (args, key) => {
    let renderColor = (str) => String(str);
    if (!args.color) {
      if (key === 'REST') {
    ...
    if (args.width) {
      if (args.width < 0) {
        args.renderColor = function (str, json) {
          const shortStr = str.length > Math.abs(args.width) ?
            '...' + str.substring(Math.max(str.length + args.width + 3, 0)) : str.padStart(-args.width)
          return renderColor(shortStr, json)
        }
      } else {
        args.renderColor = function (str, json) {
          const shortStr = str.length > args.width ?
            str.substring(0, Math.max(args.width - 3, 0)) + '...' : str.padEnd(args.width)
          return renderColor( shortStr, json)
        }
      }
    } else {
