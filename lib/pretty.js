  ...
  ld.forEach(keys, (args, key) => {
    ...
    if (!args.color) {
      if (key === 'REST') {
        ...
        renderColor = (str) => {
          return typeof(str) === 'object' ? JSON.stringify(str, null, 2) : str;
        }
      }
    } else {
      if (args.values) {
    ...
    if (args.width) {
      if (args.width < 0) {
        args.renderColor = function (str, json) {
		  const realStr = typeof str === 'string' ? str : JSON.stringify(str);
          const shortStr = realStr.length > Math.abs(args.width) ?
            '...' + realStr.substring(Math.max(realStr.length + args.width + 3, 0)) : realStr.padStart(-args.width)
          return renderColor(shortStr, json)
        }
      } else {
        args.renderColor = function (str, json) {
		  const realStr = typeof str === 'string' ? str : JSON.stringify(str);
          const shortStr = realStr.length > args.width ?
            realStr.substring(0, Math.max(args.width - 3, 0)) + '...' : JSON.stringify(realStr).padEnd(args.width)
          return renderColor( shortStr, json)
        }
      }
    } else {
